...
def generate_test_vectors(bit_size, num_test_cases):
    ...
    for i in range(0, num_test_cases):
        ...
        packaged = (result,ribitsa,rbitsb,op)
...
def main():
    ...
    result_filename = "BIT_" + bit_size + ".results"
    vector_file = "BIT_" + bit_size + ".testvector"

    rfile = open(result_filename, "w")
    vfile = open(vector_file, "w")

    for vector in test_vectors:
        rfile.writeline(vector[0])
        formatted = "%d %d %d" % (vector[1],vector[2],vector[3])
        vfile.writeline(formatted)
