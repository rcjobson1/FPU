...
#set_register_type -flip_flop DFF_X1 -exact

if {$clk_name_list != {} } {
   group_path -name "reg2reg" -critical_range $critical_range -from [ all_registers -clock_pins ] -to [ all_registers -data_pins ]
   group_path -name "in2reg"  -critical_range $critical_range -from $non_ideal_inputs -to [ all_registers -data_pins ]
   group_path -name "reg2out" -critical_range $critical_range -from [ all_registers -clock_pins ] -to [all_outputs]
}
   group_path -name "in2out"  -critical_range $critical_range -from $non_ideal_inputs -to [all_outputs]
...
if {[info exists project_cfg] && ($project_cfg == 1) } {
   ...
   #compile -inc
   # try if compile_ultra works
   #set_optimize_registers true -check_design -verbose -sync_transform multiclass -design Pipeline
   optimize_registers -check_design -verbose -clock clk -edge rise -delay_threshold 1 
   compile_ultra -retime -incremental -timing_high_effort_script   
}
...
redirect [format "%s%s%s" log/ $top_module _hier_timing.rep] { report_timing -max_paths $num_of_path}
...
#optimize_registers -clock clk -edge rise -delay_threshold 1 
...
redirect [format "%s%s%s" log/ $top_module _timing.rep] { report_timing -max_paths $num_of_path -nworst $num_of_path}
redirect -append [format "%s%s%s" log/ $top_module _timing.rep] { report_timing -true}

check_timing
set true_delay_prove_true_backtrack_limit $num_of_path 

report_timing -group clk -max_paths $num_of_path
report_timing -true
write_sdf -version 2.1 -context verilog arm-pipeline.sdf
write_sdc arm-pipeline.sdc
